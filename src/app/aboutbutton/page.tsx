...
const AboutPage = () => {
  ...
  const y = useTransform(scrollYProgress, [0, 1], ['0%', '-50%']); // Used for floating particles
  ...
  const experiences: Experience[] = [
    ...
    },
    {
      ...
      description: "Implemented responsive UIs and optimized web performance for client projects.",
      responsibilities: [
    ...
    },
    {
      ...
      company: "Startup Hub",
      description: "Contributed to various web projects and learned modern development practices.",
      responsibilities: [
  ...
  return (
    <div className="px-[8%] py-20 max-2xl:px-10 max-lg:px-5 min-h-screen bg-black text-white" ref={containerRef}>
      <main className="px-4 py-20">
        ...
        >
          <div className="max-w-4xl mx-auto text-center">
            ...
            >
              <p>Passionate developer with 5+ years of experience in creating digital solutions.</p>
              <p className="opacity-80">Specializing in scalable web applications with modern technologies.</p>
            </motion.div>
          </div>
        </motion.section>
...
const ExperienceItem = ({ year, role, company, description, responsibilities, achievements, tech, index, isActive, onClick }: ExperienceItemProps & { isActive: boolean, onClick: () => void }) => (
  <motion.div
    className={`relative md:flex items-center gap-8 group cursor-pointer ${index % 2 === 1 ? 'md:flex-row-reverse' : ''}`}
    initial={{ opacity: 0, y: 50 }}
  ...
  >
    ...
    <div className={`md:w-1/2 md:${index % 2 === 0 ? 'pr-8 text-right' : 'pl-8'}`}>
      <motion.div
          ...
          {isActive && (
            ...
            >
              {responsibilities && responsibilities.map((res, i) => (
                <p key={i} className="text-sm text-gray-300">• {res}</p>
              ))}
              {achievements && achievements.map((ach, i) => (
                <p key={i} className="text-sm text-green-400">✔ {ach}</p>
              ))}
              {tech && (
                <div className="flex flex-wrap gap-2 mt-2">
                  {tech.map((t, i) => (
                    <span key={i} className="px-3 py-1 text-sm bg-white/5 rounded-full">{t}</span>
                  ))}
                </div>
              )}
            </motion.div>
